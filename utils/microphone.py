...
class AudioUtilities(object):
    ...
    def GetMicrophones(self):
        deviceEnumerator = comtypes.CoCreateInstance(CLSID_MMDeviceEnumerator, IMMDeviceEnumerator,
                                                     comtypes.CLSCTX_INPROC_SERVER)
        audio_input_devices = deviceEnumerator.GetDefaultAudioEndpoint(EDataFlow.eCapture.value,
                                                                       DEVICE_STATE.ACTIVE.value)
        if audio_input_devices is None:
            print("ERROR: No Active Audio Input devices detected!")
            sys.exit(0)
        print("Audio Devices Found: {}".format(audio_input_devices))
    ...
    def MuteMicrophone(self):
        ...
        try:
            ...
            print("Microphone Should be mute")
            self.is_muted = True
